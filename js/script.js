...
function tablefor(event,jouranl){
    let table = [0,0,0,0]
...
for (let ivent of journal){
    if (ivent.events.includes("peanuts")&&!ivent.events.includes("brushed teeth")){
        ivent.events.push("peanuts teeth")
    }
}
...
function filter(array , test , key,condtion){
   var  list = []
   for (let a of array){
        if(test(a,key,condtion)){
            list.push(a)
        }
   }
   return list
...
function Verify(element){
    if (element.living==true){
        return true
    }
}
function Test(element,key,condtion){
    if (element[key]==condtion){
        return true
    }
}
...
function findElement(element_name){
...
function char_Count(element){
    sum=0
    for (let i of element.ranges){
        let difference = 0  
        for (let x of i){
             difference = Math.abs((difference-x))
...
var big = 0
function theBiggestCharcterCount(sum) {
    if (sum>big){
        big = sum
